...
__global__ void create_edges(edge** edges, int* other_edges, int E)
{
    ...
    if (index < E) {
        ...
        edges[index]->connect[0] = 0;
        edges[index]->connect[1] = 0;
    }
}
...
{
    if (argc < 6) {
        std::cerr << "Usage: " << argv[0] << " <filename> <update_file> <vertices> <edges> <updates>" << std::endl;
        return 1;
    }
    ...
    load_Graph(filename, updatefile, V, E, U, &edges, updates);

    printf("V:%d E:%d U:%d\n", V, E, U);
    SCC_tree* tree = new SCC_tree(V, E);

    edge* d_org_edges;
    cudaMalloc(&d_org_edges, E * sizeof(edge));
    ...
    create_edges<<<blocks, BLOCKSIZE>>>(edges_ptr, d_edges, E);
    ...
    int scc_count;
    int* d_scc_count;
    cudaMalloc(&d_scc_count, sizeof(int));
    get_scc_count<<<1, 1>>>(tree->master_node, d_scc_count);
    cudaDeviceSynchronize();
    ...
    get_nodes_size<<<1, 1>>>(tree->master_node, d_scc_count);
    cudaDeviceSynchronize();
    cudaMemcpy(&scc_count, d_scc_count, sizeof(int), cudaMemcpyDeviceToHost);
